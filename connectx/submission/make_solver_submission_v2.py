"""Build submission_alphazero_solver_gen8_v2.py (TB-style root filter integration)."""
...
SOLVER_AND_AGENT = r'''
# --- bitboard exact solver (Pascal Pons layout, v2 integration) ---
_BB_W = 7
_BB_H = 6
...
def build_solver_v2_submission(
    template_path: Path,
    output_path: Path,
) -> Path:
    ...
    # Remove original _drop, _winner, _search, agent — replaced by v2 block.
    body_text = re.sub(
        r"def _drop\(board, action, mark, rows, columns\):[\s\S]*?return False\n\n\n",
        "",
        body_text,
        count=1,
    )
    ...
    header = [
        ...
        "# note: gen8 rollback + TB-style solver root filter (v2)",
        "",
    ]

    # Preserve WEIGHTS from template.
    template_text = template_path.read_text(encoding="utf-8")
    weights_line = next(line for line in template_text.splitlines() if line.startswith("WEIGHTS_B64"))

    content = "\n".join(header + [weights_line, ""] + body_text.splitlines() + [SOLVER_AND_AGENT.strip(), ""])
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("submission/submission_alphazero_solver_gen8_v2.py"),
    )
    args = parser.parse_args()
    path = build_solver_v2_submission(args.template, args.output)
    print(f"Wrote {path}")
    return 0
